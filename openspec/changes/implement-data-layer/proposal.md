...
- **New `Core` Swift Package**: Create a dedicated local Swift Package (`Core/`) that depends on both `Domain` and `SwiftData`, housing all persistence and live client logic.
...
- **New Package**: `Core/` Swift Package added alongside `Domain/`, `Features/`, and `Common/`.
- **Dependency Graph**: `Core` depends on `Domain` and `SwiftData`. The main app target (`NeuLedger`) will import `Core` to register live dependencies. `Features` continues to depend only on `Domain` (no direct SwiftData import).
