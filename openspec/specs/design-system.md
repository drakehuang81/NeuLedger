...
- **Design File**: `openspec/changes/define-architecture/designs/neuledger-design-system.pen`
...
- **THEN** use:
  - `TabView` with `.tabViewStyle(.tabBarOnly)` for bottom tabs (Dashboard, Analysis, Settings).
...
### Requirement: Split TabBar (Floating Navigation)
The app uses a **Split Capsule TabBar** pattern for navigation.

#### Scenario: TabBar Structure
- **Left Capsule**: Home | Analysis | Settings (main navigation tabs)
- **Right Capsule**: Context action (Search by default)
- **Behavior**: Floats 16pt above the bottom safe area. May react to scroll direction.

---

### Requirement: System States

#### Scenario: Custom Alert
- Glass-morphic popup with centered icon, title, body, and action buttons.
- Width: ~300pt, centered horizontally.
- Corner radius: 24pt with background blur.

#### Scenario: Empty State
- Centered illustration (SF Symbol), title, and subtitle.
- Used for: empty transaction lists, no search results, offline state.
...
- **THEN** use the "Welcome" illustration (e.g., `tray.fill`)
- **AND** title: "尚無交易"
...
- **THEN** use the "Chart" illustration (e.g., `chart.pie.fill`)
- **AND** title: "此期間無資料"
