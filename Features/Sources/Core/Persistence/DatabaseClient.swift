...
// MARK: - DatabaseClient Analytics Helpers
//
// CRUD helpers (makeContext / fetch / add / update / deleteFirst) were retired
// in Phase 1.6 — all Repositories now route reads/writes through SwiftDataStore.
// The analytics endpoints below stay here until Phase 5 moves them into
// AnalyticsUseCase per docs/architecture.md §5.
extension DatabaseClient {

    /// Creates a fresh `ModelContext` from the shared container for the
    /// analytics helpers below. This is a private convenience — outside
    /// callers must NOT reach into the container themselves.
