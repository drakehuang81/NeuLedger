...
struct TransactionClientTests {
    ...
    // Budget-warning tests moved to BudgetUseCase as of Phase 4.2/4.3 — the
    // notification side-effect now lives in BudgetUseCase.evaluateAfterTransaction,
    // invoked by LedgerUseCase.record/update under architecture.md §3.1
    // Scenario B. Phase 5 will add integration tests covering that flow
    // end-to-end (BudgetWarningPolicy already has 8 pure-function tests).
}
